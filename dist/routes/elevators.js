...
router.get('/station', async (req, res) => {
    try {
        ...
        const result = await db_1.default.query(`SELECT * FROM elevator_info 
       WHERE station_name ILIKE $1
       ${city ? 'AND city ILIKE $2' : ''}
       ORDER BY created_at DESC`, city ? [`%${station_name}%`, `%${city}%`] : [`%${station_name}%`]);
...
router.get('/city', async (req, res) => {
    try {
        ...
        const result = await db_1.default.query(`SELECT * FROM elevator_info 
       WHERE city ILIKE $1
       ORDER BY station_name, created_at DESC`, [`%${city}%`]);
        res.json(result.rows);
    }
    catch (error) {
        console.error('Database error:', error);
...
router.post('/', async (req, res) => {
    try {
        const { station_name, transit_line, city, entrance_location, notes, is_working, firebase_uid } = req.body;
        if (!station_name || !firebase_uid) {
            res.status(400).json({ error: 'Station name and user ID are required' });
            return;
        }
        const result = await db_1.default.query(`INSERT INTO elevator_info 
        (station_name, transit_line, city, entrance_location, notes, is_working, firebase_uid)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING *`, [station_name, transit_line, city, entrance_location, notes, is_working ?? true, firebase_uid]);
...
router.get('/summary', async (req, res) => {
    try {
       ...
       ORDER BY station_name`, city ? [`%${city}%`] : []);
        res.json(result.rows);
    }
    catch (error) {
        console.error('Database error:', error);
