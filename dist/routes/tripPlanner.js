...
router.get('/search', async (req, res) => {
    try {
        ...
        const dbResult = await db_1.default.query(
       ...
       GROUP BY b.google_place_id, b.overall_accessibility_score`, [placeIds]);
        const dbMap = {};
        ...
        if (isChicago) {
            //CTA elevator status:
            try {
                const ctaResponse = await fetch('https://lapi.transitchicago.com/api/1.0/alerts.aspx?outputType=JSON&accessibility=true');
                const ctaData = await ctaResponse.json();
                const alerts = ctaData?.CTAAlerts?.Alert || [];
                elevatorOutages = alerts
                    .filter((a) => a.ShortDescription?.toLowerCase().includes('elevator'))
                    .map((a) => a.ShortDescription);
            }
            catch {
                elevatorOutages = [];
            }
            ...
            try {
                const metraResponse = await fetch('https://gtfsapi.metrarail.com/gtfs/alerts', { headers: { 'Accept': 'application/json' } });
                if (metraResponse.ok) {
                    ...
                    metraOutages = entities
                        ...
                        .map((e) => e?.alert?.header_text?.translation?.[0]?.text || 'Metra accessibility alert');
                }
            }
            catch {
                metraOutages = [];
        ...
        if (isNYC) {
            // MTA elevator status
            try {
                const mtaResponse = await fetch('https://api-endpoint.mta.info/Dataservice/mtagtfsfeeds/nyct%2Fnyct_ene_equipments.json', { headers: { 'x-api-key': '' } });
                if (mtaResponse.ok) {
                    const mtaData = await mtaResponse.json();
                    const equipment = mtaData?.equipment || mtaData || [];
                    const outOfService = Array.isArray(equipment)
                        ? equipment.filter((e) => e.equipmenttype === 'EL' &&
                            e.outagedate &&
                            !e.returntoservicedate)
                        : [];
                    elevatorOutages = outOfService.map((e) => `Elevator out of service at ${e.station} (${e.trainno} line)`);
                }
            }
            catch {
                // Backup if MTA endpoint fails (GTFS-RT alerts) (less detailed but better than nothing)
                try {
                    const mtaAlertsResponse = await fetch('https://collector-otp-prod.camsys-apps.com/realtime/gtfsrt/ALL/alerts.json?type=json&apikey=otp_key');
                    if (mtaAlertsResponse.ok) {
                        ...
                        elevatorOutages = entities
                            ...
                            .map((e) => e?.alert?.header_text?.translation?.[0]?.text || 'MTA accessibility alert')
                            .slice(0, 10);
                    }
                }
                catch {
        ...
        if (isSeattle) {
            ...
            try {
                const stResponse = await fetch('https://www.soundtransit.org/ride-with-us/service-alerts');
                if (stResponse.ok) {
                    elevatorOutages = [];
                }
            }
        ...
        const scoredPlaces = detailedPlaces.map(place => {
            ...
            maxScore += 25;
            if (place.wheelchair_accessible_entrance === true) {
                score += 25;
            ...
            const nearbyStations = CTA_STATIONS.filter(station => distanceMiles(placeLat, placeLng, station.lat, station.lng) <= 0.5);
            //Match outages to nearby L stations
            const nearbyCtaOutages = elevatorOutages.filter(outage => nearbyStations.some(station => outage.toLowerCase().includes(station.name.toLowerCase())));
            //Match metra outages to nearby area (within 1 mile)
            const nearbyMetraOutages = metraOutages.filter(() => distanceMiles(placeLat, placeLng, 41.8819, -87.6278) <= 1.0);
            if (totalOutages === 0) {
                score += 20;
            ...
            return {
                ...
                google_accessible: place.wheelchair_accessible_entrance,
...
router.post('/share', async (req, res) => {
    try {
        ...
        await db_1.default.query(`INSERT INTO shared_searches (share_id, query, city, results, cta_outages)
       VALUES ($1, $2, $3, $4, $5)`, [shareId, query, city, JSON.stringify(results), cta_outages]);
...
router.get('/share/:shareId', async (req, res) => {
    try {
        ...
        const result = await db_1.default.query('SELECT * FROM shared_searches WHERE share_id = $1', [shareId]);
