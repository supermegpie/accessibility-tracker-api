...
function computeScore(tier1_q1, tier1_q2, tier2_q1, tier2_q2, tier3_features, warnings) {
    ...
    if (featureCount >= 3 && warningCount === 0)
        return 5;
    // Score 4 (has 2+ features and no warnings)
    if (featureCount >= 2 && warningCount === 0)
        return 4;
...
router.post('/', async (req, res) => {
    try {
        const { review_id, business_id, firebase_uid, category, tier1_q1, tier1_q2, tier2_q1, tier2_q2, tier3_features, warnings } = req.body;
        ...
        const otherReviews = await db_1.default.query(`SELECT tier3_features, warnings FROM review_features
       WHERE business_id = $1 AND firebase_uid = $2 AND category != $3`, [business_id, firebase_uid, category]);
        const confirmedElsewhere = new Set();
        const warnedElsewhere = new Set();
        for (const row of otherReviews.rows) {
            (row.tier3_features || []).forEach((f) => confirmedElsewhere.add(f));
            (row.warnings || []).forEach((w) => warnedElsewhere.add(w));
        }
        ...
        const computed_score = computeScore(tier1_q1, tier1_q2, tier2_q1, tier2_q2, resolvedFeatures, resolvedWarnings);
        const result = await db_1.default.query(`INSERT INTO review_features
       ...
       RETURNING *`, [
        ...
        if (scoreCol) {
         ...
         WHERE id = $1`, [business_id, category]);
        }
