...
router.get('/search', async (req, res) => {
    try {
        ...
        const stopTypes = customStops
            ? customStops.split(',').map(s => s.trim())
            : VIBE_STOPS[vibe] || VIBE_STOPS['Foodie'];
        //Geocode the city
        ...
        for (const stopType of stopTypes) {
            ...
            for (const place of candidates) {
                ...
                const reviewResult = await db_1.default.query(`SELECT AVG(overall_score) as avg_score, COUNT(*) as review_count
           FROM reviews r
           JOIN businesses b ON r.business_id = b.id
           WHERE b.google_place_id = $1`, [place.place_id]);
                const communityScore = reviewResult.rows[0].avg_score;
                const reviewCount = reviewResult.rows[0].review_count;
                if (communityScore >= 4)
                    score += 25;
                else if (communityScore >= 3)
                    score += 12;
                if (reviewCount > 0)
                    score += 10;
                //Give proximity bonus, closer to last stop is better
                const dist = distanceMiles(lastLocation.lat, lastLocation.lng, place.geometry.location.lat, place.geometry.location.lng);
                if (dist < 0.25)
                    score += 15;
                else if (dist < 0.5)
                    score += 10;
            ...
            if (bestPlace) {
                ...
                const elevatorInfo = await db_1.default.query(`SELECT station_name, transit_line, entrance_location, notes, is_working
           FROM elevator_info
           WHERE city ILIKE $1
           ORDER BY created_at DESC
           LIMIT 3`, [`%${city.split(',')[0]}%`]);
                plannedStops.push({
...
router.post('/share', async (req, res) => {
    try {
        ...
        const share_id = crypto_1.default.randomBytes(6).toString('hex');
        await db_1.default.query('INSERT INTO day_plans (share_id, city, vibe, stops) VALUES ($1, $2, $3, $4)', [share_id, city, vibe, JSON.stringify(stops)]);
        res.json({ share_id });
    }
    catch (error) {
...
router.get('/share/:shareId', async (req, res) => {
    try {
        ...
        const result = await db_1.default.query('SELECT * FROM day_plans WHERE share_id = $1', [shareId]);
